...
// crate stack this just piled onto it — a bounce off loose cover on a narrow sky
// island is a death before FIGHT!. Keep clutter out of the drop columns.
//
// This is the cheap half of the same problem src/sim/maps/reach.js solves
// properly: reach.js grades where a wizard can LAND, this stops cover being
// planted in the column it lands through in the first place.
const SPAWN_CLEAR = 55;
const inSpawnColumn = (m, x) => (m.def.spawns || []).some(s => Math.abs(s.x - x) < SPAWN_CLEAR);
