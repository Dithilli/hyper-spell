...
theme('Sky Isles', { bg: '#232a40', stars: true , cover: 'crate' }, [
  ...
  { n: 'Updraft Canyon', b(m) { addStatic(m, 200, 540, 320, 34); addStatic(m, W - 200, 540, 320, 34); addStatic(m, W / 2, 300, 200, 26); }, u(m) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; if (Math.abs(b.position.x - W / 2) < 110) Body.setVelocity(b, { x: b.velocity.x, y: b.velocity.y - perSecond(0.9) }); } if (simRandom() < 0.4) particles.push({ kind: 'spark', x: W / 2 + rand(-100, 100), y: rand(300, H), vx: 0, vy: -9, life: 18, maxLife: 18, color: '#e0ffff', r: 2 }); }, s: [{ x: 200, y: 120 }, { x: W - 200, y: 120 }, { x: 340, y: 120 }, { x: W - 340, y: 120 }] },
...
theme('Deep Space', { bg: '#141426', stars: true , cover: 'pillar' }, [
  ...
  { n: 'The Core', gravity: 0.7, b(m) { addStatic(m, W / 2, 690, W, 40, { color: '#2a2a40' }); addStatic(m, 160, 420, 220, 26, { color: '#2a2a40' }); addStatic(m, W - 160, 420, 220, 26, { color: '#2a2a40' }); }, u(m) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; const dx = W / 2 - b.position.x, dy = 360 - b.position.y; const d = Math.hypot(dx, dy) || 1; if (d < 420) Body.setVelocity(b, { x: b.velocity.x + (dx / d) * perSecond(0.35), y: b.velocity.y + (dy / d) * perSecond(0.35) }); } }, s: [{ x: 160, y: 120 }, { x: W - 160, y: 120 }, { x: 300, y: 120 }, { x: W - 300, y: 120 }] },
...
theme('Storm Peaks', { bg: '#1a2030' , cover: 'pillar' }, [
  ...
  { n: 'Eye of the Storm', b(m) { addStatic(m, W / 2, 580, 340, 36, { color: '#2a3242' }); addStatic(m, 150, 500, 220, 28, { color: '#2a3242' }); addStatic(m, W - 150, 500, 220, 28, { color: '#2a3242' }); addLava(m); }, u(m, now) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; const off = b.position.x - W / 2; if (Math.abs(off) > 240) Body.setVelocity(b, { x: b.velocity.x - Math.sign(off) * perSecond(0.3), y: b.velocity.y }); } }, s: [{ x: 150, y: 120 }, { x: W - 150, y: 120 }, { x: W / 2 - 80, y: 120 }, { x: W / 2 + 80, y: 120 }] },
...
theme('The Void', { bg: '#0d0a14', stars: true , cover: 'pillar' }, [
  ...
  { n: 'Event Horizon', b(m) { const ring = [[W / 2, 660], [280, 540], [W - 280, 540], [380, 340], [W - 380, 340], [W / 2, 220]]; for (const [x, y] of ring) addStatic(m, x, y, 180, 22, { color: '#1f1830' }); }, u(m) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; const dx = W / 2 - b.position.x, dy = 430 - b.position.y; const d = Math.hypot(dx, dy) || 1; if (d > 60 && d < 500) Body.setVelocity(b, { x: b.velocity.x + (dx / d) * perSecond(0.25), y: b.velocity.y + (dy / d) * perSecond(0.25) }); } }, s: [{ x: 280, y: 120 }, { x: W - 280, y: 120 }, { x: 380, y: 120 }, { x: W - 380, y: 120 }] },
  ...
  { n: 'The Maw', b(m) { addStatic(m, 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W - 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W / 2, 300, 240, 26, { color: '#1f1830' }); }, u(m, now) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; const dx = W / 2 - b.position.x, dy = 720 - b.position.y; const d = Math.hypot(dx, dy) || 1; if (d < 480) Body.setVelocity(b, { x: b.velocity.x + (dx / d) * perSecond(0.3), y: b.velocity.y + (dy / d) * perSecond(0.3) }); } if (simRandom() < 0.4) particles.push({ kind: 'square', x: W / 2 + rand(-160, 160), y: H - rand(10, 60), vx: 0, vy: 2, life: 18, maxLife: 18, color: '#a55eea', r: 2.5 }); }, s: [{ x: 200, y: 120 }, { x: W - 200, y: 120 }, { x: 340, y: 120 }, { x: W - 340, y: 120 }] },
