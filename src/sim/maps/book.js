...
theme('The Void', { bg: '#0d0a14', stars: true , cover: 'pillar' }, [
  ...
  { n: 'The Maw', b(m) { addStatic(m, 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W - 200, 480, 320, 30, { color: '#1f1830' }); addStatic(m, W / 2, 300, 240, 26, { color: '#1f1830' }); }, u(m, now) { for (const b of Composite.allBodies(world)) { if (b.isStatic || b.isSensor) continue; const dx = W / 2 - b.position.x, dy = 720 - b.position.y; const d = Math.hypot(dx, dy) || 1; if (d < 480) Body.setVelocity(b, { x: b.velocity.x + (dx / d) * 0.3, y: b.velocity.y + (dy / d) * 0.3 }); } if (random() < 0.4) particles.push({ kind: 'square', x: W / 2 + rand(-160, 160), y: H - rand(10, 60), vx: 0, vy: 2, life: 18, maxLife: 18, color: '#a55eea', r: 2.5 }); }, s: [{ x: 200, y: 120 }, { x: W - 200, y: 120 }, { x: 340, y: 120 }, { x: W - 340, y: 120 }] },
