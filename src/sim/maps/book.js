...
import {
  ...
  createPolygon, gravityY, setAngle, setGravityY, setPosition, setVelocity,
} from '../phys/facade.js';
...
theme('The Void', { bg: '#0d0a14', stars: true , cover: 'pillar' }, [
  ...
  { n: 'Blink', b(m) { addStatic(m, W / 2, 650, W - 200, 36, { color: '#1f1830' }); addStatic(m, W / 2, 90, W - 200, 36, { color: '#1f1830' }); addStatic(m, 250, 380, 220, 24, { color: '#1f1830' }); addStatic(m, W - 250, 380, 220, 24, { color: '#1f1830' }); }, u(m, now) { const flipped = Math.floor(now / 6000) % 2 === 1; const want = flipped ? -game.baseGravity : game.baseGravity; if (gravityY() !== want) { setGravityY(want); doFlash('#ff4df0', 0.25); setBanner('BLINK', '#ff4df0', 700); } }, s: [{ x: 250, y: 300 }, { x: W - 250, y: 300 }, { x: 420, y: 300 }, { x: W - 420, y: 300 }] },
