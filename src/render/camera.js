...
//
// Ported from upstream js/camera.js (c11b4b2, boss framing from 4302ace). The
...
import { resetNameTagSlots } from './name-tags.js';

const CAM_HOME = { x: W / 2, y: H / 2, zoom: 1, tx: W / 2, ty: H / 2, tzoom: 1, shakeX: 0, shakeY: 0, rot: 0 };
...
export function setCameraEnabled(on) { camEnabled = !!on; }
...
// local players, so src/net/client.js hands us the interpolated ghost positions
// instead — see cameraPointsFromSnapshot in draw-snapshot.js).
export function cameraTarget(pts) {
...
function updateCameraShake(now) {
  ...
  setShake(shake * 0.88); // the single decay site — every render path funnels through here
}

// ---------- per-frame update ----------
export function updateCamera(now, pts) {
  if (!camEnabled) {
...
// screen (1280x720 logical, i.e. what input-keyboard.js produces) -> world
