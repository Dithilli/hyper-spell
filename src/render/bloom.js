...
//
// HEADLESS. The server imports the render tree through the bundle graph but
// never draws, and the test suite imports modules directly. Every entry point
// here returns early without a canvas or a document, so importing this file
// costs nothing and calling it headless is a no-op rather than a crash.
...
export function setBloomEnabled(on) { enabled = !!on; }
export function bloomEnabled() { return enabled; }
...
export function applyBloom(now) {
  ...
  // every channel, so 0.9 stays 0.81 while 0.3 collapses to 0.09. Three passes
