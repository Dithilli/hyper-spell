...
export function perfCount(name, value) {
  if (!PERF.on || !PERF.cur) return;
...
export function drawPerfHud(now) {
  // `ctx` guard: the headless sim host imports the render tree through the
  // bundle graph and never acquires a canvas.
...
export function perfSetEnabled(on) {
