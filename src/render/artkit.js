...
export function rgba(hex, a) { const c = _hx(hex); return `rgba(${c.r},${c.g},${c.b},${a})`; }
...
export const PARCHMENT = '#e8d2b0';
export const INK = '#2a1f38';

// ---------- shared marks ----------
// a rotating sigil ring — the signature "magic is happening here" mark
export function runeRing(ctx, x, y, r, color, now, o = {}) {
...
export function glowOrb(ctx, x, y, r, color, alpha = 1) {
...
// The optional content pack swaps this out at load (src/render/content-pack.js);
// classic scripts got that for free by reassigning the global, so it is a
// rebindable holder rather than a plain function declaration.
export let avatarVariant = baseAvatarVariant;
export function setAvatarVariant(fn) { avatarVariant = fn; }

export function drawStoryWizard(ctx, o) {
...
export const VAL_SKIN = '#c98a5a', VAL_SKIN_SH = '#7f5231', VAL_HAIR = '#3a1e12', VAL_HAIR_HI = '#7a4a28';
export const VAL_BRONZE = '#c48a2c', VAL_GOLD = '#e6bd6a', VAL_OX = '#6e2230', VAL_RIM = '#ffcf8a';
export function drawStorySorceress(ctx, o) {
  const scale = o.scale ?? 1, now = o.now || 0, facing = o.facing || 1;
  const piggy = !!o.piggy, alive = o.alive !== false && o.alive !== 0;
...
export function drawStoryGandalf(ctx, o) {
  const scale = o.scale ?? 1, now = o.now || 0, facing = o.facing || 1;
  const piggy = !!o.piggy, alive = o.alive !== false && o.alive !== 0;
...
export function drawStoryTome(ctx, o) {
...
export function drawStoryHat(ctx, o) {
...
export function drawStoryCatalyst(ctx, o) {
...
export function drawStoryCrate(ctx, o) {
  const wood = '#a6763c';
  ctx.save();
  ctx.fillStyle = wood;
...
export function drawStoryBackdrop(ctx, o) {
...
export function drawStoryTerrain(ctx, o) {
...
export function drawStorySpikes(ctx, o) {
...
export function drawSecretBoss(ctx, o) {
  const { x, y, now = 0 } = o;
  const r = o.r || 46;
...
export function drawStoryBoss(ctx, o) {
...
export function drawStoryDestructible(ctx, { x, y, w, h, angle = 0, kind = 'wood', frac = 1, color = '#6b4a2a', now = 0 }) {
...
export function drawStoryParticles(ctx, particles) {
  ctx.save();
