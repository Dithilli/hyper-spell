...
- **Ultimates never self-harm.** Every hybrid/ascension blast passes `{ selfSafe: true }` (supported by `explode`/`skyBolt`/`spawnSingularity`/`boomBolt`) — the caster gets knocked around but takes zero damage. Normal spells keep the 50% self-damage risk. Statuses must actually land (bolt/beam hit or telegraphed zone) — no auto-hit-nearest riders.
